...
export const Outrun = ({ onClose }) => {
  ...
  const padNumber = (number, numZeros, char = '0') => {
    const n = Math.abs(number);
    const zeros = Math.max(0, numZeros - Math.floor(n).toString().length);
    const zeroString = Math.pow(10, zeros).toString().substr(1).replace('0', char);
    return zeroString + n;
  };
  ...
  const KEYS = {};
  // We'll add event listeners in useEffect
  ...
  class Line {
    ...
    drawSprite(depth, layer, sprite, offset) {
      ...
      let obj =
        layer instanceof Element ? layer : this.elements[layer + 6];
  ...
  class Car {
    constructor(pos, type, lane) {
      ...
      roadRef.current.appendChild(element);
  ...
  // ------------------------------------------------------------
  // Refs for DOM elements and audio instance
  // ------------------------------------------------------------
      ...
      if (e.code === 'KeyC') {
        ...
        sleep(0)
          .then((_) => {
            ...
            if (audioRef.current) audioRef.current.play('beep');
            return sleep(1000);
          })
          .then((_) => {
            textRef.current.innerText = 2;
            if (audioRef.current) audioRef.current.play('beep');
            return sleep(1000);
          })
          .then((_) => {
            ...
            roadRef.current.style.opacity = 1;
            heroRef.current.style.display = 'block';
            hudRef.current.style.display = 'block';

            if (audioRef.current) audioRef.current.play('beep', 500);
  ...
  function update(step) {
    ...
    if (KEYS.ArrowRight) {
      heroRef.current.style.backgroundPosition = '-220px 0';
      playerX += 0.007 * step * speed;
    } else if (KEYS.ArrowLeft) {
      heroRef.current.style.backgroundPosition = '0 0';
      playerX -= 0.007 * step * speed;
    } else {
      heroRef.current.style.backgroundPosition = '-110px 0';
    }
    ...
    if (!inGame) {
      ...
      tachoRef.current.style.display = 'none';

      homeRef.current.style.display = 'block';
      roadRef.current.style.opacity = 0.4;
      textRef.current.innerText = 'INSERT COIN';

      highscores.push(lapRef.current.innerText);
      highscores.sort();
      updateHighscore();

      inGame = false;
    } else {
      timeRef.current.innerText = padNumber(countDown | 0, 3);
      scoreRef.current.innerText = padNumber(scoreVal | 0, 8);
      tachoRef.current.innerText = speed | 0;

      let cT = new Date(timestamp() - start);
      lapRef.current.innerText = `${cT.getMinutes()}'${padNumber(
        cT.getSeconds(),
        2
      )}"${padNumber(cT.getMilliseconds(), 3)}`;
    }

    // sound
    if (speed > 0) audioRef.current.play('engine', speed * 4);

    // draw cloud
    cloudOffset -= lines[startPos].curve * step * speed * 0.13;
    cloudRef.current.style.backgroundPosition = `${cloudOffset | 0}px 0`;
    ...
    for (let car of cars) {
      ...
      if (
        ...
        if (inGame) audioRef.current.play('honk');
      }
    }
  ...
  function reset() {
    ...
    homeRef.current.style.display = 'block';
    tachoRef.current.style.display = 'block';
  }

  function updateHighscore() {
    let hN = Math.min(12, highscores.length);
    for (let i = 0; i < hN; i++) {
      highscoreRef.current.children[i].innerHTML = `${padNumber(
        i + 1,
        2,
        '&nbsp;'
      )}. ${highscores[i]}`;
  ...
  function init() {
    ...
    // Ensure heroRef.current exists before accessing
    if (heroRef.current) {
      heroRef.current.style.top = height - 80 + 'px';
      heroRef.current.style.left = halfWidth - ASSETS.IMAGE.HERO.width / 2 + 'px';
      heroRef.current.style.background = `url(${ASSETS.IMAGE.HERO.src})`;
      heroRef.current.style.width = `${ASSETS.IMAGE.HERO.width}px`;
      heroRef.current.style.height = `${ASSETS.IMAGE.HERO.height}px`;
      heroRef.current.style.display = 'block'; // Ensure the hero is visible
    }

    if (cloudRef.current) {
      cloudRef.current.style.backgroundImage = `url(${ASSETS.IMAGE.SKY.src})`;
    }

    audioRef.current = new AudioClass();

    // Load audio files
    Object.keys(ASSETS.AUDIO).forEach((key) =>
      audioRef.current.load(ASSETS.AUDIO[key], key, (_) => 0)
    );
    ...
    for (let i = 0; i < 12; i++) {
      ...
      highscoreRef.current.appendChild(element);
    }
    updateHighscore();
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Close the AudioContext to stop audio playback
  ...
  // Function to handle control button presses
  const handleControlButton = (keyCode) => {
    // Simulate keydown
    KEYS[keyCode] = true;

    // Simulate keyup after a short delay to mimic a key press
    setTimeout(() => {
      KEYS[keyCode] = false;
    }, 100); // Adjust the delay as needed
  };

  const handleControlButtonDown = (keyCode) => {
    KEYS[keyCode] = true;
  };

  const handleControlButtonUp = (keyCode) => {
    KEYS[keyCode] = false;
  };

  return (
    <div className="outRunWrapper">
      {/* Close Button */}
      ...
      <div id="game" ref={gameRef}>
        <div id="road" ref={roadRef}>
          <div id="cloud" ref={cloudRef}></div>
          <div id="hero" ref={heroRef}></div>
        </div>

        <div id="hud" ref={hudRef}>
          <span id="time" className="topUI" ref={timeRef}>
            0
          </span>
          <span id="score" className="topUI" ref={scoreRef}>
            0
          </span>
          <span id="lap" className="topUI" ref={lapRef}>
            0'00"000
          </span>
          <span id="tacho" ref={tachoRef}>
            0
          </span>
        </div>
        ...
        <div id="home" ref={homeRef}>
          <h1>OutRun</h1>
          <p id="text" ref={textRef}></p>
          <div id="highscore" ref={highscoreRef}></div>
        </div>
      </div>

      {/* Updated Controls */}
      <div id="controls">
        <button className="coinButton" onClick={() => handleControlButton('KeyC')}>
          C <img src={getImagePath('games/coin.svg')} alt="Start" />
        </button>
        <button onClick={() => handleControlButton('KeyM')}>
          <span>M</span> Mute
        </button>
        <button
          onMouseDown={() => handleControlButtonDown('ArrowLeft')}
          onMouseUp={() => handleControlButtonUp('ArrowLeft')}
          onTouchStart={() => handleControlButtonDown('ArrowLeft')}
          onTouchEnd={() => handleControlButtonUp('ArrowLeft')}
        >
          <span>&larr;</span>
        </button>
        <button
          onMouseDown={() => handleControlButtonDown('ArrowRight')}
          onMouseUp={() => handleControlButtonUp('ArrowRight')}
          onTouchStart={() => handleControlButtonDown('ArrowRight')}
          onTouchEnd={() => handleControlButtonUp('ArrowRight')}
        >
          <span>&rarr;</span>
        </button>
        <button
          onMouseDown={() => handleControlButtonDown('ArrowUp')}
          onMouseUp={() => handleControlButtonUp('ArrowUp')}
          onTouchStart={() => handleControlButtonDown('ArrowUp')}
          onTouchEnd={() => handleControlButtonUp('ArrowUp')}
        >
          <span>&uarr;</span> Accelerate
        </button>
        <button
          onMouseDown={() => handleControlButtonDown('ArrowDown')}
          onMouseUp={() => handleControlButtonUp('ArrowDown')}
          onTouchStart={() => handleControlButtonDown('ArrowDown')}
          onTouchEnd={() => handleControlButtonUp('ArrowDown')}
        >
